...
本文档同时描述当前的未签名手动分发和未来的签名自动更新。v0.0.5 已完成签名更新代码与 fail-closed 流水线，但仓库当前没有 Windows Authenticode 证书、Apple Developer ID 或 Tauri 更新私钥，因此默认发布通道不会读取签名 Secret。

## 0. 当前策略：未签名手动正式版

当前推送精确的 `vX.Y.Z` 标签会生成 GitHub **正式 Release**，但其安装包仍是未签名手动分发包。“正式版”只表示稳定版本通道，不表示已经通过 Windows/macOS 系统签名：

- 三处应用版本必须等于 `X.Y.Z`，标签必须精确匹配 `vX.Y.Z`；
...
- CI 会拒绝未签名 Release 中出现 `.sig` 或 `latest.json`，因此它不会进入应用内自动更新；
- Windows/macOS 会显示未知发布者或未公证警告。Release Notes 必须明确说明风险，不能把稳定版本号表述为可信签名证明。

`vX.Y.Z-test.N` 仍可用于确有需要的预发布测试，但不是当前默认发布名称。只有仓库变量 `SIGNED_RELEASES_ENABLED=true` 且后文全部证书、私钥和验证门禁均已配置时，精确版本标签才切换到签名自动更新通道；缺少任一 Secret 会 fail closed。
...
| Tauri 更新签名 | 独立公钥已内置；当前手动 Release 不生成签名，未来签名模式强制要求 CI 私钥 |
| 自动更新 | Windows / Linux AppImage 代码已实现；当前未签名 Release 不进入更新清单，macOS / DEB 保持手动 |
...
4. 当前未签名通道使用 `continuous-integration` Environment，不配置签名 Secret。未来启用签名时，再创建受保护的 `release` Environment、配置 Required reviewers，并把第 3、4 节列出的 Secrets 只放在该 Environment，不能保留同名仓库级 Secrets。
5. 提交进入 `main` 后再打 tag：`git tag vX.Y.Z && git push origin vX.Y.Z`。CI 会校验 tag、三处版本号、双语 Release Notes 以及提交可从 `origin/main` 到达；当前生成未签名安装包和 `SHA256SUMS.txt` 后公开稳定 Release。未来只有在 `SIGNED_RELEASES_ENABLED=true` 时才进入签名矩阵、生成 `.sig`/`latest.json` 并执行密码学验签。
...
普通分支 / PR 的 Actions Artifact 仍是不带系统代码签名的开发产物；当前稳定 Release 同样必须明确标注“未知发布者/未签名手动版”。只有通过上述 Authenticode 门禁的产物才可称为已签名 Windows 安装包。
...
5. **签名模式**的 GitHub Release 必须同时包含 Windows/Linux 更新包与 `.sig`、`latest.json`、所有安装包和 `SHA256SUMS.txt`，并使用 `docs/releases/vX.Y.Z.md` 的中英双语说明。当前未签名模式明确禁止 `.sig` 与 `latest.json`。两种模式都先创建 draft，上传后逐项核对远端文件名与字节数，全部一致才一次性公开；任一上传失败只会留下未公开 draft。
...
- [ ] 仓库 `refs/tags/v*` ruleset 限制创建/更新/删除；若启用签名模式，`release` Environment 启用 Required reviewers，且签名 Secrets 不存在仓库级副本
...
- [ ] 未签名模式确认没有 `.sig`/`latest.json` 且 Release Notes 明示未知发布者；签名模式则要求 Windows `signtool verify /pa /all /v` 通过
- [ ] 签名模式下，内置公钥对下载后的全部更新产物验签通过；`latest.json` 只含 Windows / Linux AppImage，全部 URL 可下载
